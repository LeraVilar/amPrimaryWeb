<template>
  <div class="workers-list" id="workers-list">
    <h2 v-if="med" class="workers-list__title">Специалистам</h2>
    <div
    ...
    >
      <img
        class="workers-list__card-image"
        :src="'https://dzhobava.com/server' + item?.image.data.attributes.url"
      />
      <div class="workers-list__info-panel">
        ...
        <div
          v-html="item?.title"
          class="workers-list__product-description"
        ></div>
        <div
          v-if="item.popupMed || item.popupGuide"
          class="workers-list__detailed-button"
        ...
        <div class="workers-list__cost-block">
          <div class="workers-list__cost-wrapper">
            <div class="workers-list__cost-label">СТОИМОСТЬ ПРОГРАММЫ</div>
            <div class="workers-list__cost">{{ item?.price }} руб.</div>
          </div>
          <a
          ...
          >
            <div class="workers-list__purchase-button">Купить программу</div>
          </a>
        </div>
        <elements-special-offer
          v-if="item?.specOffer"
          :text="item?.specOffer"
        />
      </div>
